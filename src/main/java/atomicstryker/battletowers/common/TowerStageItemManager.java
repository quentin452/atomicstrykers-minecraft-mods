    ...
    public TowerStageItemManager(String configString) {
        ...
        for (int i = 0; i < elements.length; i++) {
            ...
            if (settings.length < 5 && !settings[0].startsWith("ChestGenHook:")) {
                System.err
                    .println("Battletowers skipping invalid entry [" + elements[i].trim() + "], fix config file!");
            } else {
        ...
        if (floorHasItemsLeft() && rand.nextInt(100) < chanceToSpawn[curIndex]) {
            if (itemID[curIndex] instanceof Item) {
                ...
                result = new ItemStack(
                    ...
                    minAmount[curIndex] + rand.nextInt(maxAmount[curIndex]),
                    itemDamage[curIndex]);
                // System.out.println("Stashed new damaged ItemStack, id "+itemID[curIndex]+", "+result.getItemName()+"
                // in a BT chest.");
            } else if (itemID[curIndex] instanceof Block) {
                // System.out.println("Stashed block "+block.getLocalizedName()+" of id "+itemID[curIndex]);
                result = new ItemStack(
                    (Block) itemID[curIndex],
                    minAmount[curIndex] + rand.nextInt(maxAmount[curIndex]),
                    itemDamage[curIndex]);
                // System.out.println("Stashed new damaged Block Stack, id "+itemID[curIndex]+",
                // "+result.getItemName()+" in a BT chest.");
