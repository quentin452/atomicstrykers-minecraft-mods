...
public class ClientTickHandler {
    ...
    public void onTick(TickEvent.RenderTickEvent tick) {
        ...
        if (mc.currentScreen instanceof GuiChest) {
            List<?> ents = mc.theWorld.getEntitiesWithinAABBExcludingEntity(
                ...
                AxisAlignedBB.getBoundingBox(
                    mc.thePlayer.posX - 8D,
                    mc.thePlayer.posY - 8D,
        ...
        if (!hackFailed && mc.theWorld != null
                        ...
                        if (ents.get(i) instanceof AS_EntityGolem golem) {
                            ...
                            try {
                                progressHack = ReflectionHelper
                                    .getPrivateValue(PlayerControllerMP.class, mc.playerController, 6);
                                progress = (Float) progressHack;
                            } catch (Exception e) {
                                System.err.println(
                                    "Tell AtomicStryker to update his BattleTowers Block Break Progress Hack because: "
                                        + e);
                                hackFailed = true;
                                e.printStackTrace();
                            }
